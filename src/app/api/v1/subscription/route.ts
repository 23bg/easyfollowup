...
export async function POST(req: NextRequest) {
    try {
        ...
        if (body.action !== "create-subscription") {
            return NextResponse.json({ success: false, error: { code: "INVALID_ACTION", message: "Unsupported action" } }, { status: 400 });
        }

        if (body.planType && !isPlanType(body.planType)) {
            return NextResponse.json({ success: false, error: { code: "INVALID_PLAN", message: "Unsupported plan type" } }, { status: 400 });
        }
        ...
        if (!body.provider || body.provider === "razorpay") {
            ...
            return NextResponse.json({ success: true, data: { ...data, provider: "razorpay" } });
        }
        ...
        return NextResponse.json(
            {
                ...
                error: {
                    ...
                    message: "Stripe subscription flow is not configured yet. Use Razorpay for now.",
                },
            },
            { status: 501 }
        );
